...
export class SettingsMenu extends Scene {
  ...
  preload() {
    // Load assets
    if (!this.textures.exists('wooden_sign')) {
      this.load.image('wooden_sign', 'assets/wooden_sign.png')
    }

    this.load.audio('menu-select', 'assets/audio/sound_effect-menu_select.mp3')
  }
  ...
  create() {
    ...
    // Background image
    const backgroundImage = this.add.image(512, 384, 'background-menu')
    backgroundImage.setDepth(-10)

    // Create wooden sign for title
    const signY = this.cameras.main.height / 2 - 100
    const sign = this.add.image(this.cameras.main.width / 2, signY, 'wooden_sign').setOrigin(0.5)

    if (!this.textures.exists('wooden_sign')) {
      // Fallback if image asset is not loaded
      const signGraphics = this.add.graphics()
      signGraphics.fillStyle(0x5c4033, 1)
      signGraphics.fillRoundedRect(this.cameras.main.width / 2 - 200, signY - 40, 400, 80, 15)
      signGraphics.lineStyle(8, 0x3d2817, 1)
      signGraphics.strokeRoundedRect(this.cameras.main.width / 2 - 200, signY - 40, 400, 80, 15)
    ...
    const menuItems = [
      ...
      {
        text: this.isFullScreen ? 'SCREEN SIZE: FULLSCREEN' : 'SCREEN SIZE: WINDOWED',
    ...
    this.menuChangeSound = this.sound.add('menu-select', { volume: 0.5 })
  ...
  createMenuItems(menuItems) {
    ...
    menuItems.forEach((item, index) => {
      ...
      bg.fillRoundedRect(centerX - bgWidth / 2, buttonY - bgHeight / 2, bgWidth, bgHeight, 10)
      bg.lineStyle(5, 0x3d2314, 1)
      bg.strokeRoundedRect(centerX - bgWidth / 2, buttonY - bgHeight / 2, bgWidth, bgHeight, 10)
      bg.setAlpha(0.9)
  ...
  animateMenuEntrance() {
    ...
    const menuItems = [...this.menuTexts.map((item) => item.text), ...this.menuOptionBackground]
  ...
  moveSelection(direction) {
    ...
    this.selectedIndex = Phaser.Math.Wrap(this.selectedIndex + direction, 0, this.menuTexts.length)
  ...
  updateVolumeDisplay() {
    ...
    this.menuTexts[0].text.setText(`VOLUME: ${Math.round(this.volume * 100)}%`)
  }
  ...
  toggleFullScreen() {
    ...
    this.menuTexts[1].text.setText(
      !isFullScreenNow ? 'SCREEN SIZE: FULLSCREEN' : 'SCREEN SIZE: WINDOWED'
    )
